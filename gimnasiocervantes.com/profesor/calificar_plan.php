...
?>
  <tr <? if(esPar($i+1)){ echo ' bgcolor="#CCCCCC"';}else{ echo " bgcolor='#FFFF66'";}?>>
    ...
    <td><select name="justificadas_<?=$alumnos[$i]['identificacion']?>">
    <? for($k=0;$k<=15;$k++){?>
  	  <option value="<?=$k?>" <? if($p_actual['horas_justificadas']==$k){ echo " selected";}?>><?=$k?> Horas</option>
    <? }?>
  	</select></td>
    <td><select name="injustificadas_<?=$alumnos[$i]['identificacion']?>">
    <? for($k=0;$k<=15;$k++){?>
  	  <option value="<?=$k?>" <? if($p_actual['horas_injustificadas']==$k){ echo " selected";}?>><?=$k?> Horas</option>
    <? }?>
  	</select></td>
<?
for($l=0;$l<count($dato);$l++){
...
?>
    <td><input type="text" name="<?=$logro_evaluacion['id_logro']."-".$alumnos[$i]['identificacion']?>" id="<?=$logro_evaluacion['id_logro']."-".$alumnos[$i]['identificacion']?>" value="<?=$calificacion_logro_eval['calificacion_logro']?>" size="1" maxlength="3" onkeypress="return numeros2(event);" onpaste="return false" onblur="return comprueba_nota(this);" /></td>
    ...
    <td><a href="#" onClick="FAjax('./profesor/recomendacion.php?identificacion=<?=$alumnos[$i]['identificacion']?>&nva_recomendacion=1','area_trabajo','','post');">Recomendaciones>></a></td>
  </tr>
<?
}
?>
...
?>
		...
		<table width="670px" cellpadding=5 cellspacing=0 border="0">
			...
			<tr>
				<td align="center"><input class="button_send" name="guardar_calificacion" id="guardar_calificacion" type="button" value="GUARDAR CALIFICACIONES" onClick="valida_guarda_notas(this.form);"><!--&nbsp;&nbsp;<input class="button_send" name="recomendacion" id="recomendacion" type="button" value="RECOMENDACIONES" onClick="FAjax('./profesor/recomendacion.php','area_trabajo','','post');">--></td>
			</tr>
        </table>