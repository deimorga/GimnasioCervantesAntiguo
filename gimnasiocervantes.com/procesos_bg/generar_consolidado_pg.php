...
for($i=0;$i<count($gupo_asignatura);$i++){
	$existe=getPlanGestor($gupo_asignatura[$i]['id_grupo_asignatura'],$_REQUEST['periodo']);
	if($existe){
		echo "<br>OK: ".$gupo_asignatura[$i]['id_grupo_asignatura'];
	}else{
		$insert=setPlanGestor($gupo_asignatura[$i]['id_grupo_asignatura'],$_REQUEST['periodo']);
