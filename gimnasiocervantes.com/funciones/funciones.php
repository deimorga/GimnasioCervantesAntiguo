...
######################################################
	...
	function getCandidato(){
		$sql="select * from tbl_alumno where candidatura=1 order by tarjeton";
		return getSQL($sql,true);
	}
	
	function setCandidato($candidato,$candidatura, $partido, $tarjeton){
		$sql="update tbl_alumno set
	candidatura=$candidatura, partido='$partido', tarjeton='$tarjeton' where identificacion=$candidato";
		//echo $sql;
		return updateSQL($sql);
	}

	function setVotoCandidato($alumno_voto,$candidato_voto){
		$sql="update tbl_alumno set
	voto='$candidato_voto' where identificacion=$alumno_voto";
		//echo $sql;die();
		return updateSQL($sql);
	}

	function getVotoAlumnoId($id){
		
	$sql="select voto from tbl_alumno where identificacion=$id";
	//echo $sql;
	return getSQL($sql);
...
######################################################
	...
	function getModulosUsuario($id){
		$sql="select a.*, b.* from tbl_usuario_modulo a, tbl_modulo b where a.id_modulo=b.id_modulo and a.id_usuario='$id' order by b.id_modulo asc";
		//echo $sql;
		return getSQL($sql,true);
	}
...
######################################################
	...
	function getEnlacesModulo($id){
		$sql="select * from tbl_enlace where id_modulo='$id' order by id_enlace asc";
		//echo $sql;
		return getSQL($sql,true);
	}
...
///////////////////////////////SEDES////////////////////////////
	function getSedes(){
		$sql="select * from tbl_sede order by id_sede asc";
		return getSQL($sql,true);
	}

	function getSedeId($id){
		$sql="select * from tbl_sede where id_sede=$id";
		return getSQL($sql);
	}

	function setSede($nom,$desc,$dir,$lat,$lng){
		$sql="insert into tbl_sede (nombre, descripcion, direccion, lat, lng, tipos_nombre) values ('$nom','desc', '$dir','$lat','$lng', 2)";
		return insertSQL($sql);
	}
	
	function updSede($nom,$desc,$dir,$lat,$lng,$id){
		$sql="update tbl_sede set nombre='$nom', descripcion='$desc', direccion='$dir', lat='$lat', lng='$lng' where id_sede=$id";
		return updateSQL($sql);
	}
	
	function delSede($id){
		$sql="delete from tbl_sede where id_sede=$id";
		//die($sql);
		return deleteSQL($sql);
	}

/////////////////////////////ENLACES WEB///////////////////////////////
	function getEnlacesPrincipal($directo=0){
		$sql="select * from tbl_enlace_principal where solo_acceso_directo IN($directo) order by id_enlace_principal desc";
		return getSQL($sql,true);
	}

	function getEnlacePrincipalId($id){
		$sql="select * from tbl_enlace_principal where id_enlace_principal=$id";
		return getSQL($sql);
	}

	function setEnlacePrincipal($nom,$desc){
		$sql="insert into tbl_enlace_principal (nombre_enlace_principal, descripcion_enlace_principal) values ('$nom','$desc')";
		//die($sql);
		return insertSQL($sql);
	}
	
	function updEnlacePrincipal($nom,$desc,$id){
		$sql="update tbl_enlace_principal set nombre_enlace_principal='$nom', descripcion_enlace_principal='$desc' where id_enlace_principal=$id";
		return updateSQL($sql);
	}
	
	function delEnlacePrincipal($id){
		$sql="delete from tbl_enlace_principal where id_enlace_principal=$id";
		//die($sql);
		return deleteSQL($sql);
	}

/////////////////////////////CONTENIDOS ENLACE///////////////////////////////
	function getContenidosEnlaceIcono(){
		$sql="select a.*, b.* from tbl_contenido_enlace a, tbl_tipo_contenido_enlace b where a.id_tipo_contenido_enlace=b.id_tipo_contenido_enlace and LENGTH(a.icono)>0 order by a.nombre_contenido_enlace asc";
		//die($sql);
		return getSQL($sql,true);
	}

	function getContenidosEnlace($id){
		$sql="select a.*, b.* from tbl_contenido_enlace a, tbl_tipo_contenido_enlace b where a.id_tipo_contenido_enlace=b.id_tipo_contenido_enlace and a.id_enlace_principal=$id order by a.nombre_contenido_enlace asc";
		//die($sql);
		return getSQL($sql,true);
	}

	function getContenidoEnlaceId($id){
		$sql="select a.*, b.* from tbl_contenido_enlace a, tbl_tipo_contenido_enlace b where a.id_tipo_contenido_enlace=b.id_tipo_contenido_enlace and a.id_contenido_enlace=$id";
		return getSQL($sql);
	}

	function setContenidoEnlace($nom,$desc,$id, $tipo_contenido, $img){
		$sql="insert into tbl_contenido_enlace (nombre_contenido_enlace, html_code, id_enlace_principal, id_tipo_contenido_enlace, icono) values ('$nom','$desc', $id, $tipo_contenido, '$img')";
		//die($sql);
		return insertSQL($sql);
	}
	
	function updContenidoEnlace($nom,$desc,$id, $img){
		$sql="update tbl_contenido_enlace set nombre_contenido_enlace='$nom', html_code='$desc', icono='$img' where id_contenido_enlace=$id";
		//die($sql);
		return updateSQL($sql);
	}
	
	function delContenidoEnlace($id){
		$sql="delete from tbl_contenido_enlace where id_contenido_enlace=$id";
		//die($sql);
		return deleteSQL($sql);
	}

/////////////////////////////TIPO CONTENIDOS ENLACE///////////////////////////////
	function getTiposContenidoEnlace(){
		$sql="select * from tbl_tipo_contenido_enlace where id_tipo_contenido_enlace>0 order by id_tipo_contenido_enlace asc";
		return getSQL($sql,true);
	}
...
$sql="select a.*, b.* from tbl_asignatura a, tbl_area b where a.id_area=b.id_area and a.activo=1 order by id_asignatura asc";
//echo $sql;
return getSQL($sql,true);
...
function getAsignaturasGrupoPuntaje($id, $periodo){
	
$sql="select a.*, b.*, c.*, d.*, e.* from tbl_grupo_asignatura a, tbl_area b, tbl_grupo c, tbl_asignatura d, tbl_plan_gestor e where a.id_grupo=$id and e.id_periodo_academico=$periodo and a.id_grupo=c.id_grupo and a.id_asignatura=d.id_asignatura and d.id_area=b.id_area and a.id_grupo_asignatura=e.id_grupo_asignatura and d.id_asignatura not in (28) order by d.id_area, a.id_asignatura asc";
//die($sql);
return getSQL($sql, true);
...
function getAsignaturaGrupoInforme($grupo, $director, $asignatura){
	...
	$sql.=" order by a.id_asignatura, b.id_grupo, d.id_profesor asc";
	return getSQL($sql, true);	
}
...
function getLogroId($id){
	$sql="select * from tbl_logro where id_logro=$id";
	return getSQL($sql,false);
}

function setLogro($desc,$porcentaje,$pg){
	$sql="insert into tbl_logro (id_plan_gestor,logro,porcentaje) values ($pg,'$desc',$porcentaje)";
	return insertSQL($sql);
}

function actualizaLogro($desc,$porcentaje,$id){
	$sql="update tbl_logro set logro='$desc',porcentaje=$porcentaje where id_logro=$id";
	return updateSQL($sql);
}

function getPorcentajeLogrosPlanGestor($id){
	$sql="select SUM(porcentaje) as total_porcentaje from tbl_logro where id_plan_gestor=$id;";
	return getSQL($sql);
}

function getCountDesempaniosGrupoAsignatura($grupo,$periodo,$asignatura){
	$sql="select COUNT(a.id_calificacion_asignatura) as num_desempenios, c.nombre_asignatura from tbl_calificacion_asignatura a, tbl_alumno b, tbl_asignatura c where a.id_alumno=b.identificacion and a.id_asignatura=c.id_asignatura and a.id_asignatura=$asignatura and a.id_periodo=$periodo and b.id_grupo=$grupo group by a.desempenio_asignatura;";
	//echo $sql;
	return getSQL($sql);
}
...
######################################################
	...
	function getComunicados($id){
 	...
 	function getComunicadosAlumnoPeriodo($alumno, $mes, $anio){
		//echo $mes."+++++++".$anio;
		$sql="select a.*, b.* from tbl_comunicado_alumno a, tbl_comunicado b where a.id_alumno=$alumno and a.id_comunicado=b.id_comunicado AND MONTH(fecha_aplica)='$mes' AND YEAR(fecha_aplica)='$anio'";
		//echo $sql;
		return getSQL($sql,true);
	}

 	function getGroupComunicadosAlumnoPeriodo($alumno, $mes, $anio){
		//echo $mes."+++++++".$anio;
		$sql="select count(a.id_comunicado_alumno) AS num_comunicados, c.color_tipo_comunicado, DAY(b.fecha_aplica) AS dia from tbl_comunicado_alumno a, tbl_comunicado b, tbl_tipo_comunicado c where a.id_alumno=$alumno and a.id_comunicado=b.id_comunicado and b.id_tipo_comunicado=c.id_tipo_comunicado AND MONTH(b.fecha_aplica)='$mes' AND YEAR(b.fecha_aplica)='$anio' group by c.id_tipo_comunicado, DAY(b.fecha_aplica) asc";
		//echo $sql;
		return getSQL($sql,true);
	}

 	function getComunicadosAlumnoDia($alumno, $fecha){
		//echo "+++++++".$fecha;
		$sql="select a.*, c.*, b.*, d.* from tbl_comunicado_alumno a, tbl_comunicado b, tbl_tipo_comunicado c, tbl_usuario d where a.id_alumno=$alumno and a.id_comunicado=b.id_comunicado and b.id_tipo_comunicado=c.id_tipo_comunicado and b.id_usuario=d.identificacion_usuario AND b.fecha_aplica='$fecha'";
		//die($sql);
		return getSQL($sql,true);
	}
	...
	function setComunicado($asunto,$contenido,$id,$fecha_aplica,$tipo){
		$fecha = date ( 'Y-m-d' ,strtotime ( '-5 hour' , strtotime ( date("Y-m-d H:i:s") ) ) );
		$sql="insert into tbl_comunicado 
	(asunto, comunicado, fecha, id_usuario, fecha_aplica, id_tipo_comunicado)
	values
	('$asunto', '$contenido', '$fecha', $id, '$fecha_aplica', $tipo)";
		//die($sql);
		return insertReturnSQL($sql);
	}
	...
	function getTipoComunicado(){
		$sql="select * from tbl_tipo_comunicado";
		return getSQL($sql,true);
	}
	
	function getTipoComunicadoId($id){
		$sql="select * from tbl_tipo_comunicado where id_tipo_comunicado=$id";
		return getSQL($sql);
	}
	
	function setTipoComunicado($nom,$desc,$color,$activo){
		$sql="insert into tbl_tipo_comunicado 
	(nombre_tipo_comunicado, descripcion_tipo_comunicado, color_tipo_comunicado, activo)
	values
	('$nom', '$desc', '$color', $activo)";
		//die($sql);
		return insertReturnSQL($sql);
	}
	
	function updTipoComunicado($id,$nom,$desc,$color,$activo){
		$sql="update tbl_tipo_comunicado set nombre_tipo_comunicado='$nom', descripcion_tipo_comunicado='$desc', color_tipo_comunicado='$color', activo=$activo where id_tipo_comunicado=$id";
		//echo $sql;die();
		return updateSQL($sql);
	}
...
######################################################
	...
	function getSeccionesPlanGestor(){
		$sql="select * from tbl_seccion_plan_gestor where activo=1";
		return getSQL($sql,true);
	}

######################################################
######       DATO SECCION PLAN GESTOR           ######
######################################################
	
	function getDatosSeccionPlanGestor($seccion,$plan){
		$sql="select a.*, b.*, c.* from tbl_dato_seccion_plan_gestor a, tbl_seccion_plan_gestor b, tbl_plan_gestor c where a.id_seccion_plan_gestor=b.id_seccion_plan_gestor and a.id_plan_gestor=c.id_plan_gestor and a.id_plan_gestor=$plan and a.id_seccion_plan_gestor=$seccion";
		//echo $sql;
		return getSQL($sql,true);
	}
	
	function getDatoSeccionPlanGestorId($id){
		$sql="select a.*, b.*, c.* from tbl_dato_seccion_plan_gestor a, tbl_seccion_plan_gestor b, tbl_plan_gestor c where a.id_seccion_plan_gestor=b.id_seccion_plan_gestor and a.id_plan_gestor=c.id_plan_gestor and a.id_dato_seccion_plan_gestor=$id";
		//echo $sql;
		return getSQL($sql);
	}
	
	function setDatoSeccionPlanGestor($sec,$desc,$pg,$user){
		$sql="insert into tbl_dato_seccion_plan_gestor (id_seccion_plan_gestor,descripcion_dato_seccion_plan_gestor,id_plan_gestor,create_user) values ($sec,'$desc',$pg,$user)";
		//echo $sql;
		return insertReturnSQL($sql);
	}
	
	function delDatoSeccionPlanGestor($id){
		$sql="delete from tbl_dato_seccion_plan_gestor where id_dato_seccion_plan_gestor='$id'";
		//echo $sql;
		return deleteSQL($sql);
	}
	
	//actualizaDatoSeccionPlanGestor
	function actualizaDatoSeccionPlanGestor($desc,$id){
		$sql="update tbl_dato_seccion_plan_gestor set descripcion_dato_seccion_plan_gestor='$desc' where id_dato_seccion_plan_gestor=$id";
		//echo $sql;
		return updateSQL($sql);
	}
