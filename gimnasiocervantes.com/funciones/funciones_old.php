...
######################################################
	...
	$sql.=" and a.id_grado=b.id_grado and a.id_grupo=c.id_grupo order by a.nombre_alumno asc";
	//echo $sql;
	return getSQL($sql,true);
	...
	function setAlumno($identificacion, $expedida, $nombre, $fecha, $lugar, $sangre, $seguro, $nombre_eps, $procedencia, $grado){
	
		$existe=getAlumnoId($identificacion);
		
		if($existe){
			$sql="update tbl_alumno set
	lugar_expedicion='$expedida', nombre_alumno='$nombre', fecha_nacimiento='$fecha', lugar_nacimiento='$lugar', rh='$sangre', nivel_sisben='$seguro', eps='$nombre_eps', procedencia='$procedencia', id_grado=$grado
	where identificacion=$identificacion";
	//echo $sql;
			return updateSQL($sql);
		}else{
			$sql="insert into tbl_alumno 
	(identificacion, lugar_expedicion, nombre_alumno, fecha_nacimiento, lugar_nacimiento, rh, nivel_sisben, eps, procedencia, id_grado)
	values
	($identificacion, '$expedida', '$nombre', '$fecha', '$lugar', '$sangre', '$seguro', '$nombre_eps', '$procedencia', $grado)";
...
######################################################
	...
	function getCirculares(){
		$sql="select * from tbl_circular order by fecha_circular, id_circular desc";
		return getSQL($sql,true);
	}
...
$sql="select * from tbl_profesor order by nombre_profesor asc";
//echo $sql;
return getSQL($sql,true);
...
function getPlanesGestores($grupo, $profesor, $asignatura, $periodo){
	...
	$sql.=" order by a.id_profesor, a.id_grupo asc ";
	//echo $sql;
	return getSQL($sql,true);	
}
...
######################################################
	...
	function getPlanEntrenamientoId($id){
		$sql="select a.*, b.* from tbl_plan_entrenamiento a, tbl_grupo_asignatura b, tbl_grupo c where a.id_plan_entrenamiento=$id and a.id_grupo_asignatura=b.id_grupo_asignatura and b.id_grupo=c.id_grupo";
		//echo $sql;
		return getSQL($sql);
	}
...
######################################################
	...
	function setEjercicio($nombre_ejercicio, $descripcion_ejercicio, $id_plan_entrenamiento){
		$sql="insert into tbl_ejercicio 
	(nombre_ejercicio, descripcion_ejercicio, id_plan_entrenamiento)
	values
	('$nombre_ejercicio', '$descripcion_ejercicio', $id_plan_entrenamiento)";
		//echo $sql;
		return insertReturnSQL($sql);
...
######################################################
	...
	function getConceptosPagados($fechaini,$fechafin,$id,$nombre="",$concepto,$factura, $grupo){
		...
		$sql="select a.*, b.*, c.*, d.* from tbl_concepto_pago_alumno a, tbl_concepto_pago b, tbl_factura c, tbl_alumno d where a.estado_concepto_pago=1 and a.id_concepto_pago=b.id_concepto_pago and a.factura=c.id_factura and a.id_alumno=d.identificacion and c.fecha_pago between '$fechaini' and '$fechafin' and a.id_alumno like '%$id%' and d.nombre_alumno like '%$nombre%' and a.factura like '%$factura%' and a.id_concepto_pago like '%$concepto%' and d.id_grupo like '%$grupo%' order by a.factura asc";
		//echo $sql;
		return getSQL($sql,true);
	}
	...
	function getConceptosAlumnoPagados($alumno_pago){
		...
		$sql="select a.*, b.*, c.* from tbl_concepto_pago_alumno a, tbl_concepto_pago b, tbl_factura c where a.id_alumno=$alumno_pago and a.estado_concepto_pago=1 and a.id_concepto_pago=b.id_concepto_pago and a.factura=c.id_factura order by a.factura desc";
		//echo $sql;
		return getSQL($sql,true);
	}
	...
	values
	($alumno_pago, $concepto_pago, NOW(), $valor)";
	//echo $sql;
		return insertSQL($sql);
	}
	...
	values
	($alumno_pago, $id_concepto, NOW(), $valor, '$periodo')";
	//echo $sql;
		return insertSQL($sql);
	}
...
######################################################
	...
	values
	('$nombre_gasto',$valor_gasto,NOW(),'$observaciones_gasto',$user_id)";
		return insertSQL($sql);
	}
...
######################################################
	...
	function setFactura($user_id,$alumno_pago){
		
		$sql="insert into tbl_factura (fecha_pago, id_usuario, id_alumno)values(NOW(), $user_id, $alumno_pago)";
		return insertReturnSQL($sql);
	}
