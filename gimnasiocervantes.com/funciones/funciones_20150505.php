...
######################################################
	...
	$sql.=" and a.id_grado=b.id_grado and a.id_grupo=c.id_grupo order by a.id_grado, a.id_grupo, a.nombre_alumno asc";
	//echo $sql;
	return getSQL($sql,true);
...
1000686068
	...
	function getAlumnosMatriculados($anio){
		$sql="SELECT a.identificacion, a.nombre_alumno, b.nombre_grado, c.factura 
FROM tbl_alumno a, tbl_grado b, tbl_concepto_pago_alumno c, tbl_factura d 
WHERE a.identificacion=c.id_alumno AND a.id_grado=b.id_grado and c.factura=d.id_factura AND c.id_concepto_pago=8 ORDER BY a.id_grado, a.nombre_alumno ASC";
		//echo $sql;
		return getSQL($sql,true);
	}
...
######################################################
	...
	function getCircularesGrupoId($id){
		$sql="select a.*, b.* from tbl_circular a, tbl_grupo b where a.tipo=1 and a.dirigida='$id' and a.dirigida=b.id_grupo order by a.id_circular desc";
