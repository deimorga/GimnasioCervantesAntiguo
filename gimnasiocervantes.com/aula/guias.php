...
if(isset($_GET['borrar'])){
	//echo $_GET['borrar'];
	$borro=delCircular($_GET['borrar']);
	if($borro){
		echo "<script>alert('Se borro correctamente...');</script>";
	}else{
		echo "<script>alert('No se borro la guia, contacte al administrador...');</script>";
	}
}

$circulares=getCircularesId($_SESSION["user_id"]);
?>
...
?>
  <tr <? if(esPar($i+1)){ echo ' bgcolor="#CCCCCC"';}else{ echo " bgcolor='#FFFF66'";}?> style="color:#000; font-size:18px">
    ...
    <td><a href='./circulares/<?=$circulares[$i]['id_circular'].$circulares[$i]['archivo']?>' target='_blank'>Descargar</a><br /><a href="#" class="link_right" onClick="if(confirm('Seguro de eliminar la guia?')){
	FAjax('./aula/guias.php?borrar=<?=$circulares[$i]['id_circular']?>','area_trabajo_alumno','','post');
	}else{
	return false;
	}
	">Borrar >></a>
