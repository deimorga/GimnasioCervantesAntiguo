...
######################################################
	...
	function getNoticias($pag){
		$fin=($pag*10);
		$ini=$fin-9;
		$sql="select * from tbl_noticia order by id_noticia desc limit $ini, $fin";
		//echo $sql;
		return getSQL($sql,true);
	}
	...
	function getCircularesId($id){
		$sql="select a.*, b.*, c.* from tbl_circular a, tbl_grupo b, tbl_profesor c where a.tipo=1 and a.emisor='$id' and a.emisor=c.id_profesor and a.dirigida=b.id_grupo order by a.id_circular desc";
	...
	function getCircularesGrupoId($id){
		$sql="select a.*, b.*, c.* from tbl_circular a, tbl_grupo b, tbl_profesor c where a.tipo=1 and a.dirigida='$id' and a.dirigida=b.id_grupo and a.emisor=c.id_profesor order by a.id_circular desc";
