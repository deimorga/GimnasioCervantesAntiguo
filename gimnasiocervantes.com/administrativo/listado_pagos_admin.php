...
$conceptos=getConceptosPagados($_REQUEST['fechaini'],$_REQUEST['fechafin'],$_SESSION['esturiante_matriculado'],$_REQUEST['nombre_alumno'],$_REQUEST['concepto_pago'],$_REQUEST['id_factura'],$_REQUEST['grupo']);
}else{
...
for($i=0;$i<count($conceptos);$i++){
	$tot_efectivo=$tot_efectivo+$conceptos[$i]['valor_cancelado'];
?>
  <tr>
...
<h2>Listado de consignaciones.</h2>
<table class="table table-hover table-striped table-bordered">
  <thead>
    <tr>
    <th>No. factura</th>
...
<?
if(isset($_SESSION['esturiante_matriculado'])){
$conceptos2=getConceptosConsignados($_REQUEST['fechaini'],$_REQUEST['fechafin'],$_SESSION['esturiante_matriculado'],$_REQUEST['nombre_alumno'],$_REQUEST['concepto_pago'],$_REQUEST['id_factura'],$_REQUEST['grupo']);
}else{
$conceptos2=getConceptosConsignados($_REQUEST['fechaini'],$_REQUEST['fechafin'],$_REQUEST['id_alumno'],$_REQUEST['nombre_alumno'],$_REQUEST['concepto_pago'],$_REQUEST['id_factura'],$_REQUEST['grupo']);
}
...
for($i=0;$i<count($conceptos2);$i++){
	$tot_consignaciones=$tot_consignaciones+$conceptos2[$i]['valor_cancelado'];
?>
  <tr>
    <td><?=$conceptos2[$i]['id_factura']?></td>
    <td><?
    echo $conceptos2[$i]['nombre_concepto_pago'];
	if($conceptos2[$i]['tipo_concepto']>=3 && $conceptos2[$i]['tipo_concepto']<6){ 
echo " periodo ".$conceptos2[$i]['periodo_facturado'];		  }
	?>
	</td>
    ...
    <td align="right"><?=$conceptos2[$i]['valor_cancelado']?></td>
  </tr>
<?
}}
?>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td align="center"><input type="button" value="IMPRIMIR" class="btn-sm btn-primary" onclick="window.open('./administrativo/listado_pagos_admin.php?fechaini=<?=$_REQUEST['fechaini']?>&fechafin=<?=$_REQUEST['fechafin']?>&id_alumno=<?=$_REQUEST['id_alumno']?>&nombre_alumno=<?=$_REQUEST['nombre_alumno']?>&concepto_pago=<?=$_REQUEST['concepto_pago']?>&id_factura<?=$_REQUEST['id_factura']?>&descargar_pagos=1');"/></td>
  </tr>
</table>
