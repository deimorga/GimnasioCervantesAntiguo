...
1000686068
	...
	$sql="select a.*, b.*, c.* from tbl_alumno a, tbl_grado b, tbl_alumno_anio c where c.identificacion=$id and c.anio_lectivo=$anio AND c.identificacion=a.identificacion and a.id_grado=b.id_grado";
	//echo $sql;
	return getSQL($sql);
...
######################################################
	...
	function getConceptosConsignados($fechaini,$fechafin,$id,$nombre="",$concepto,$factura, $grupo){
		...
		$sql="select a.*, b.*, c.*, d.* from tbl_concepto_pago_alumno a, tbl_concepto_pago b, tbl_factura c, tbl_alumno d where a.estado_concepto_pago=1 and c.tipo_pago=2 and a.id_concepto_pago=b.id_concepto_pago and a.factura=c.id_factura and a.id_alumno=d.identificacion and c.fecha_pago between '$fechaini' and '$fechafin' and a.id_alumno like '%$id%' and d.nombre_alumno like '%$nombre%' and a.factura like '%$factura%' and a.id_concepto_pago like '%$concepto%' and d.id_grupo like '%$grupo%' order by a.factura asc";
		//echo $sql;
		return getSQL($sql,true);
	}
...
######################################################
	...
	function setFactura($user_id,$alumno_pago,$tipo){
		...
		$sql="insert into tbl_factura (fecha_pago, id_usuario, id_alumno, tipo_pago)values(NOW(), $user_id, $alumno_pago, $tipo)";
		//die($sql);
		return insertReturnSQL($sql);
	}
